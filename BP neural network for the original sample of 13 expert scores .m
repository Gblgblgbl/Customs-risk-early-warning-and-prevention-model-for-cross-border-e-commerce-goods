% 加载数据
num = xlsread('risk.xlsx', 'Sheet1', 'A1:R17'); % 数据范围为 17 行，18 列
input_data = num(:, 1:17); % 输入数据（前17列）
expected_output = num(:, 18); % 期望输出（第18列）

% 划分训练集和测试集
input_train = input_data(1:13, :)'; % 前13行样本作为训练输入，并转置
output_train = expected_output(1:13)'; % 前13行的期望值作为训练输出
input_test = input_data'; % 所有17行样本作为测试输入，并转置
output_test = expected_output'; % 所有17行的期望值
...
% 可视化期望值和网络输出值对比
figure;
plot(1:17, output_test, '-o', 'DisplayName', '期望值');
hold on;
plot(1:17, network_output, '-x', 'DisplayName', '网络输出值');
xlabel('样本编号');
ylabel('输出值');
title('网络输出与期望值对比');
legend;
